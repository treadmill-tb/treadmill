...
pub struct AppendArgs {
    ...
    /// A layer blob as `ROLE=PATH`, placed on top of ROLE's chain (which only a
    /// qcow2 blob can go on), or starting it if the lower has no such role.
    ...
    /// metadata.
    #[arg(long = "layer", value_name = "ROLE=PATH")]
    layers: Vec<LayerArg>,
...
pub fn append(args: &AppendArgs) -> anyhow::Result<()> {
    ...
    for layer in &args.layers {
        let blob = out.store_layer(&layer.path)?;
        builder
