...
#
# Port of the legacy `images/netboot-raspberrypi-nbd/default.nix`. Raspberry Pi
# OS = Debian 13 "trixie", booted disklessly over NBD: a raw FAT boot partition
# (TFTP-served firmware/kernel/cmdline) plus an ext4 root served as a qcow2 NBD
# volume. The customization is done by booting the stock RPi OS image under TCG
# `qemu-system-aarch64` with a custom init that patches `cmdline.txt`, installs
# the puppet daemon + nbd-client, etc.
#
# OCI-migration changes vs. the original:
#   * the puppet binary comes from the in-tree static aarch64 musl package
#     (`puppet` arg) instead of the bespoke `images/lib/puppet.nix` fenix build;
#   * `rustup-init` is a `pkgs.fetchurl` FOD (offline-eval-safe), not
#     `builtins.fetchurl`;
...
let
  ...
  # retained for a future netboot run harness (§6.6); the base/overlay image
  # builds only consume `out`.
