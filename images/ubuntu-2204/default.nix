...
#
# Port of the legacy `images/vm-ubuntu-2204-amd64-uefi/default.nix` through
# `vmTools.makeImageFromDebDist` (same package list / createRootFS / postInstall:
# grub-efi, networking, the `tml` user, ssh, dbus, the `tml-puppet.service`
# unit, rustup), with the two mechanical changes the OCI migration requires:
#   (a) the puppet binary comes from the in-tree static package (`puppet` arg)
#       instead of the bespoke `images/lib/puppet.nix` fenix musl build (I8);
#   (b) the output is a `mkTreadmillImage` OCI layout instead of the bespoke
#       TOML store shell.
# The final `qemu-img convert -c` compression of the qcow2 is preserved.
{
  pkgs,
  lib,
  mediaTypes,
...
mkTreadmillImage {
  ...
  layers = [
    {
      path = "${ubuntuImage}/disk-image.qcow2";
