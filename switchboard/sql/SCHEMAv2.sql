...
(
    ...
    appointed_at     timestamp without time zone not null,
    added_by_user_id uuid                        not null references users (user_id) on delete no action
);
...
-- Metadata
    created_by_user_id       uuid references users (user_id) on delete no action,
    created_at               timestamp without time zone not null,
    last_modified_by_user_id uuid references users (user_id) on delete no action,
