...
-- be used to restore the switchboard's functional state, and record the final exit status of all finalized jobs. It
-- does not aim to record the concrete state of the job, beyond the minimum that is required for state reconstruction.

-- Restart policy dictates the number of times a job can be restarted automatically by the switchboard.
create type tml_switchboard.restart_policy as
(
    remaining_restart_count integer
);
-- Functional state is an internal property of jobs in the switchboard that describes how the job is treated by the
-- switchboard; it can be treated as a simplification of execution status that only concerns itself with the
...
create type tml_switchboard.exit_status as enum (
    ...
    -- EXTERNAL
    'job_canceled',
    'workload_finished_success', -- easily distinguished exit status, as opposed to a single workload_terminated status
...
-- It is necessary to record the ID of the supervisor to which the job has been dispatched. If both switchboard and
-- supervisor fail, then upon supervisor reconnection, the switchboard must be able to discern the failure of the job;
-- otherwise, the only recourse of the user is to wait for the job to time out.
...
-- For bookkeeping purposes; for ease of use, for now this works by just putting `last_updated_at = DEFAULT` on UPDATE queries, as per
-- https://www.morling.dev/blog/last-updated-columns-with-postgres/. The original intention was to have something like an ON UPDATE trigger,
-- but since it seems a bit messy to do with Postgres, it's being left alone for now.
...
-- running => started_at, R.O.S.I != null
    constraint valid_queued_implication check (functional_state != 'queued' or
                                               (started_at is null and dispatched_on_supervisor_id is null)),
    constraint valid_dispatched_implication check (functional_state != 'dispatched' or
...
-- Exit status vs. host output
    constraint exit_status_allows_host_output check (case
                                                         ...
                                                         when exit_status = 'supervisor_dropped_job'
                                                             then host_output is null
                                                         when exit_status is null then host_output is null
                                                         else true
        end),
-- exit status != null <=> functional state == finalized
    constraint exit_status_nullity_iso_finalized check ((exit_status is not null) = (functional_state = 'finalized'))
);
...
-- This is a table of its own because per-job parameter key uniqueness is a useful property to enforce.
...
-- create type tml_switchboard.execution_status as enum (
--     'queued',
--     'scheduled',
--     'initializing',
--     'ready',
...
(
    ...
    check (job_event ? 'event_type'
        and (job_event ->> 'event_type' = 'state_transition'
