...
pub async fn login(
    ...
    let user_lookup_result =
        lookup_user_password_hash(app_state.pool(), &login_request.user_identifier).await;
    ...
    let (token, expires_at) = sql::api_token::insert_token(
        app_state.pool(),
        user.user_id,
        app_state.config().service.default_token_timeout,
        TokenPerms::InheritsUserPerms,
    )
    .await
    .map_err(|e| {
        tracing::error!(
...
async fn lookup_user_password_hash<'c, E: PgExecutor<'c>>(
    ...
    let TempRecord {
        ...
        TempRecord,
        r#"SELECT user_id, password_hash, locked
