...
// ) -> Result<(), sqlx::Error> {
//     let (resume_job_id, restart_job_id, image_id): (Option<JobId>, Option<JobId>, Option<Vec<u8>>) =
//         match job_request.init_spec {
//             JobInitSpec::ResumeJob {
//                 job_id: init_spec_job_id,
//             } => (Some(init_spec_job_id.into()), None, None),
//             JobInitSpec::RestartJob {
//                 job_id: init_spec_job_id,
//             } => {
...
//                 .await?;
//                 (
//                     predecessor.resume_job_id.map(|id| id.into()),
//                     Some(init_spec_job_id.into()),
...
//         "#,
//         job_id as JobId,
//         resume_job_id as Option<JobId>,
//         restart_job_id as Option<JobId>,
//         image_id,
...
pub struct SqlJob {
    ...
    pub restart_job_id: Option<JobId>,
...
impl SqlJob {
    ...
    // TODO: Weird constraints, look into this!
    //
...
pub async fn get(job_id: JobId, conn: impl PgExecutor<'_>) -> Result<SqlJob, sqlx::Error> {
    sqlx::query_as!(
        SqlJob,
        r#"
        SELECT
            job_id as "job_id!: JobId",
            resume_job_id as "resume_job_id?: JobId",
            restart_job_id as "restart_job_id?: JobId",
            ...
            started_at,
            assigned_supervisor as "assigned_supervisor?: SupervisorId",
