...
/// A security token is a 32-byte string generated by a cryptographically secure PRNG.
///
...
pub enum HttpDecodeError {
    ...
    #[error("Expected token to be byte string of length 32")]
    Length,
}
impl TryFrom<Bearer> for SecurityToken {
    ...
    fn try_from(value: Bearer) -> Result<Self, Self::Error> {
        ...
        let a: [u8; 32] = v.try_into().map_err(|_| HttpDecodeError::Length)?;
        Ok(Self(AuthToken(a)))
    }
}
